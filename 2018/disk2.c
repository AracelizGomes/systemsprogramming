...
typedef struct disk {
    ...
    int diskcap;
    int    writewait; // make write priority
...
void* readjob(void * arg_orig) 
{
    ...
    for(i=0; i<ITERATIONS; i++){
        ...
        pthread_mutex_lock(&shared_info->mutex);
        while(shared_info->numwritejobs > 0 || shared_info->writewait > 0 || shared_info->numreadjobs == shared_info->diskcap){  // must wait
