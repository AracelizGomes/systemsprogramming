...
void * writejob(void * arg_orig) 
{
    ...
    for(i=0; i<ITERATIONS; i++){
        ...
        while(shared_info->numreadjobs > 0 || shared_info->numwritejobs == shared_info->diskcap) {  // must wait
...
void* readjob(void * arg_orig) 
{
    ...
    for(i=0; i<ITERATIONS; i++){
        ...
        pthread_mutex_lock(&shared_info->mutex);
        //while loop that waits for write jobs to finish before readjobs begin 
        
        while(shared_info->numwritejobs > 0 || shared_info->numreadjobs == shared_info->diskcap) {  // must wait
        ...
        if(shared_info->numreadjobs==0 ) 
            pthread_cond_broadcast(&shared_info->available); //unblock the threads when read jobs finish
...
int main(int argc, char *argv[]) 
{
    ...
    disk_t shared_info;
    shared_info.numwritejobs = shared_info.numreadjobs = 0; //jobs active (not active yet) initializes
